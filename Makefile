...
# Container image coordinates; must match deploy/portainer-stack.yml. The deploy
# workflow publishes the same names to GHCR.
IMAGE_OWNER ?= nicolaslallier
IMAGE_TAG   ?= latest
IMAGE_PREFIX := ghcr.io/$(IMAGE_OWNER)/darkangel
...
docker-build-frontend: ## Build the frontend image (nginx serving the built SPA)
	docker build -t $(IMAGE_PREFIX)-frontend:$(IMAGE_TAG) $(FRONTEND)

stack-up: ## Run the Portainer stack locally on FRONTEND_PORT (default 8080)
	IMAGE_OWNER=$(IMAGE_OWNER) IMAGE_TAG=$(IMAGE_TAG) $(COMPOSE) -f $(STACK) up -d

stack-down: ## Stop the local stack
	IMAGE_OWNER=$(IMAGE_OWNER) IMAGE_TAG=$(IMAGE_TAG) $(COMPOSE) -f $(STACK) down

stack-logs: ## Follow the local stack logs
	IMAGE_OWNER=$(IMAGE_OWNER) IMAGE_TAG=$(IMAGE_TAG) $(COMPOSE) -f $(STACK) logs -f
