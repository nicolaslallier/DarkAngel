...
umask 077
{
  ...
  echo "#"
  echo "# It lives here and not in .env because this token can register runners"
  echo "# on the repository -- the same reasoning that keeps PORTAINER_API_KEY in"
  echo "# .portainer.env rather than in a file handed to a container. Gitignored;"
  echo "# never commit it."
  printf 'GH_RUNNER_TOKEN=%s\n' "$token"
  echo "# GH_RUNNER_NAME=darkangel-host        # how the runner is named in GitHub's UI"
...
echo
echo "  Next:  make runner-up       # registers it with the label 'darkangel'"
echo "         make runner-status   # the container here, and GitHub's view of it"
echo
echo "  Then set the DEPLOY_RUNNER repository variable to 'darkangel', so"
echo "  .github/workflows/deploy.yml runs its deploy job on this runner."
