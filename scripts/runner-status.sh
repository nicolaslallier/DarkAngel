...
[ -f .runner.env ] || {
  echo "runner-status: .runner.env not found -- run 'make runner-env'" >&2
  exit 1
}
# shellcheck disable=SC1091
set -a; . ./.runner.env; set +a

REPO="${GH_RUNNER_REPO:-nicolaslallier/DarkAngel}"
LABEL="${GH_RUNNER_LABEL:-darkangel}"
BUSY_ONLY=0
[ "${1:-}" = "--busy" ] && BUSY_ONLY=1
...
# not as "unknown", which reads like a network blip.
#
...
# scripts/gen-runner-env.sh and scripts/portainer-stack.sh.
json=""
http_code=""
gh_state=""
...
if [ "$BUSY_ONLY" = 1 ]; then
  ...
  # Scoped to the deploy label rather than to every runner on the repo: that
  # label is the contract with the DEPLOY_RUNNER repository variable, and so
  # names exactly the runner these targets recreate.
  busy="$(jq -r --arg l "$LABEL" \
    '[.runners[] | select(.busy) | select(.labels[].name == $l)] | length' <<<"$json")"
  [ "$busy" = 0 ] && exit 0
  jq -r --arg l "$LABEL" '.runners[] | select(.busy) | select(.labels[].name == $l)
...
if [ "$gh_state" = bad-credential ] || [ "$gh_state" = bad-scope ]; then
  ...
  # else here reports it: `make runner-up` succeeds, the container is up, and
  # a push to main queues its deploy silently because GitHub does not treat
  # "no runner matches these labels" as an error.
  echo "  ${gh_why}"
  echo
...
MSG
    ...
    jq -e --arg l "$LABEL" '[.runners[] | select(.labels[].name == $l)] | length > 0' \
      >/dev/null <<<"$json" || cat <<MSG
  none of them carries the '${LABEL}' label that the DEPLOY_RUNNER repository
  variable selects on -- the deploy job matches nothing and queues silently.
MSG
  fi
fi
